...
public class FmqReporter implements MetricReporter, Scheduled {
  ...
  private static final List<Double> HISTORGRAM_QUANTILES = Arrays.asList(.5, .75, .95, .98, .99, .999);
  @VisibleForTesting
  static final char SCOPE_SEPARATOR = '_';
  @VisibleForTesting
  static final String SCOPE_PREFIX = "flink" + SCOPE_SEPARATOR;
  ...
  @VisibleForTesting
  private final FMQProducer fmqProducer;
  ...
  public FmqReporter(
    String bootstrapServers, String topic, String currentSite, String jobId, String jobName,
  ...
  public void open(MetricConfig config) {
    boolean filterLabelValueCharacters = FmqReporterOptions.getBoolean(config, FILTER_LABEL_VALUE_CHARACTER);
  ...
  public void notifyOfAddedMetric(Metric metric, String metricName, MetricGroup group) {
    ...
    for (final Map.Entry<String, String> dimension : group.getAllVariables().entrySet()) {
      ...
      final String dimKey = CHARACTER_FILTER.filterCharacters(key.substring(1, key.length() - 1));
  ...
  public void report() {
    ...
    for (String reportJSON : sendJSONList) {
      ProducerMessage message = new ProducerMessage(reportJSON.getBytes(StandardCharsets.UTF_8));
      SendResult fmqSendResult = fmqProducer.sendSync(topic, message);
      if (fmqSendResult.isFailed()) {
        LOGGER.error("flink metric send fmq failed, message:{}, {}", reportJSON, fmqSendResult);
      }
    }
  }
  ...
  private List<String> getGaugesReport(long currentTimeMillis) {
    ...
    for (Map.Entry<Gauge<?>, FmqMetricInfo> gauge : gaugeMetrics.entrySet()) {
      ...
      if (value == null) {
        LOGGER.debug("Gauge {} is null-valued, defaulting to 0.", gauge);
      ...
      LOGGER.debug("Invalid type for Gauge {}: {}, only number types and booleans are supported by this reporter.",
      ...
      try {
        ...
        LOGGER.error("flink gauge metric serialize failed", e);
      }
    }
    return reportList;
  }
  ...
  private List<String> getCountersReport(long currentTimeMillis) {
    ...
    for (Map.Entry<Counter, FmqMetricInfo> counter : counterMetrics.entrySet()) {
      ...
      try {
        ...
        LOGGER.error("flink counter metric serialize failed", e);
      }
    }
    return reportList;
  }
  ...
  private List<String> getMetersReport(long currentTimeMillis) {
    ...
    for (Map.Entry<Meter, FmqMetricInfo> meter : meterMetrics.entrySet()) {
      ...
      try {
        ...
        LOGGER.error("flink meter metric serialize failed", e);
      }
    }
    return reportList;
  }
  ...
  private List<String> getHistogramsReport(long currentTimeMillis) {
    ...
    for (Map.Entry<Histogram, FmqMetricInfo> histogram : histogramMetrics.entrySet()) {
      try {
        ...
        LOGGER.error("flink histogram metric serialize failed", e);
      }
    }
    return reportList;
  }
